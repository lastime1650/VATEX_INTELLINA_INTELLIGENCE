...
class SigValidator:
    def __init__(self, catalog=None, openssl_path:str=None):
    ...
    '''
        SUCCESS : Hash_algorithm String
        Failed  : None
    '''
    def verify_pe(self, pe, rebuilt=False)->Optional[str]:
    ...
    def verify_signature(self, cert):
        ...
        self.save_data(self.file_signature, signature) # 저장

        # openssl asn1parse -inform DER -in /tmp/tmp0UGO2s
        process = subprocess.Popen([self.openssl_path, 'asn1parse', '-inform', 'DER', '-in', self.file_signature],
        ...
        if where:
            ...
            process = subprocess.Popen([self.openssl_path, 'smime', '-verify', '-inform', 'DER', '-in', "temp",
            ...
            #os.remove(self.file_signed_data)
    ...
    def get_digest_from_signature(self, signature):
        ...
        OID_sha384 = binascii.unhexlify('608648016503040202')    # sha384  
        OID_sha512 = binascii.unhexlify('608648016503040203')    # sha512 
        ...
        if match:
            ...
            elif oid_algorithm == OID_sha384:
                return 'sha384', digest
            elif oid_algorithm == OID_sha512:
...
def Check_sign_exe(file:any=None, openssl_path:str=r"openssl")->Optional[bool]:
    try:
        sig = SigValidator(
            openssl_path=openssl_path
        )
        
        if isinstance(file, bytes): # -> 바이너리
            pe = pefile.PE(
                data=file,
                fast_load=True
            )
            ...
            raise Exception("파일 타입이 올바르지 않습니다.")

        return sig.verify_pe(pe)
    except Exception as e:
        print(e)
        return None

#print( Check_sign_exe("notepad.exe") )